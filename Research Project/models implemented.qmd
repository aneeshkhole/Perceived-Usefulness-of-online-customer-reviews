...
#we should omit this from our project, I think - PC
...
mod = LogisticRegression(penalty='l2',max_iter = 150,class_weight={0:.2,1:.8})
...
```{python}
#can't remember what I was doing here - commenting out for now.
# X_test['prob'] = y_pred 
# (X_test[X_test['prob'] == 1]).index
# t.loc[(X_test[X_test['prob'] == 1]).index]
```
...
#we're not going to do this one for our project.
#we can examine this or other models if we have substantial issues with KNN, MLR, or Log Reg.
...
```{python SVMC2}
#to be removed unless absolutely needed
