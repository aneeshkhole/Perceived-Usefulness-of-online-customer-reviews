...
resLG = pd.DataFrame(
    lg_pred.head(30).describe()
)
resLG = resLG[resLG.index.isin(['mean','std'])].copy()
resLG['model'] = "Logistic Regression (Tuned)"
resLG = resLG.reset_index()

# resMLR = pd.DataFrame(
#     mlr_pred.head(30).describe()
# )
...
resMLR = resMLR[resMLR.index.isin(['mean','std'])].copy()
resMLR['model'] = "Multiple Linear Reguression"
resMLR = resMLR.reset_index()
summary = pd.concat([resMLR,resLG],ignore_index=True)
...
resSVM = resSVM[resSVM.index.isin(['mean','std'])].copy()
resSVM['model'] = "SVM-Poly (Tuned)"
resSVM = resSVM.reset_index()
summary = pd.concat([summary,resSVM],ignore_index=True)
summary.rename({'level_0':'metric'},axis=1,inplace=True)


display(
    summary[['model','metric',
        'review_subjectivity', 'review_polarity','review_star_rating',
